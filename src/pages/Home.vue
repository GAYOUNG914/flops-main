<template>
  <div>
    <!-- Intro Screen -->
    <IntroScreen v-if="showIntro" :screenStartWithBlur="['.intro', 'header.main-header']" @complete="onIntroComplete"/>
                ...
                <div class="sticky-inner">
                  <div class="img-glob" ref="globeRef">
                    ...
                    <video
                      ...
                      playsinline
                      loading="lazy"
                      src="/portal-ui/main/final333.webm"
                    >
                    </video>
                  </div>
                </div>
              ...
              <transition name="fade-slide" mode="out-in">
                <div :key="insightSlides[insightCurrentIndex].video" class="video-wrap">
                  <video ref="insightVideoRef" muted loop playsinline >
                    ...
                    <source src="/portal-ui/main/insight-video.mp4" type="video/mp4">
                  </video>
                </div>
              </transition>
            </div>
                    ...
                    <div class="accordion-container">
                      <div class="accordion-item">
                        ...
                        <div class="accordion-bottom">
                          ...
                          <div class="desc">
                            <p class="b2_ko_s">LLM 기반 실시간 수율/품질 분석 도구 접근 및 실행 솔루션 <br/>
                              tool 교육시간 없이 자연어 대화로 바로 활용 가능한 분석 시스템</p>
                            <Button class="btn-go" minWidth="86" size="32" backgroundColor="waferblue1" fontColor="white">GO!</Button>
                          </div>
                        </div>
                      </div>
                      <div class="accordion-item">
...
import IntroScreen from '../portal-ui/components/IntroScreen.vue'
import Button from '../portal-ui/components/Button.vue'
...
const videoSrc = ref('') // 초기값 빈 문자열로 설정
...
function analysisSection(){
  ...
  for(let i = 0; i < $accordions.length; i++){
    ...
    // image-graphic img 초기 상태: grayscale(100%)
    const imgEl = accordion.querySelector('.image-graphic img');
    if (imgEl) {
      gsap.set(imgEl, { filter: 'grayscale(100%)' });
    }

    gsap.set($accordions[i].querySelector('.accordion-bottom .desc'),{
        opacity: 0,
        maxHeight: 0,
        pointerEvents: 'none'
      });
  ...
  function createTimeline() {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const duration = i === $accordions.length - 1 ? 0.23 : 0.15;
      const endTime = startTime + duration + 0.05;

      //아코디언 열림
      timeline.to($accordions[i].querySelector('.accordion-content'), {
        height: "400px",
        duration: duration,
        ease: 'power1.out'
      }, startTime);

      const imgEl = $accordions[i].querySelector('.image-graphic img');
      if (imgEl) {
        timeline.to(imgEl, {
          filter: 'grayscale(0%)',
          duration: duration,
      ...
      timeline.to($accordions[i].querySelector('.accordion-bottom .desc'),{
        opacity: 1,
        maxHeight: "64px",
        duration: duration,
        ease: 'power1.out',
      ...
      timeline.to($accordions[i].querySelector('.image-graphic'), {
        top: "44%",
        right: "50%",
...
onMounted(async () => {
  ...
  nextTick(() => {
    ...
    // }else {
    //   videoSrc.value = './portal-ui/main/final333.webm';
    // }
